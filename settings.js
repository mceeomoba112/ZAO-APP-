...
async function loadUserFromSupabase() {
    ...
    try {
        const { data: userData, error } = await supabase
            .from('users')
            .select('username, name, bio, profile_picture_url')
        ...
        if (userData) {
            ...
            // Update local storage with data from database
            ...
            if (userData.profile_picture_url) {
                localStorage.setItem('profilePicture', userData.profile_picture_url);
            }
            if (userData.name) {
...
function loadUserPreferences() {
    ...
    applyDarkMode(preferences.darkMode || false);
}

// Apply dark mode styling
function applyDarkMode(enabled) {
    if (enabled) {
        document.body.style.background = 'linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 50%, #1e1e1e 100%)';
        document.body.style.color = '#ffffff';

        // Update all premium sections
        document.querySelectorAll('.premium-settings-section').forEach(section => {
            section.style.background = 'rgba(0, 0, 0, 0.3)';
            section.style.borderColor = 'rgba(255, 255, 255, 0.1)';
        });

        // Update input fields
        document.querySelectorAll('.premium-input-wrapper').forEach(wrapper => {
            wrapper.style.background = 'rgba(0, 0, 0, 0.2)';
            wrapper.style.borderColor = 'rgba(255, 255, 255, 0.1)';
        });

        // Update modals
        document.querySelectorAll('.premium-modal-content').forEach(modal => {
            modal.style.background = 'linear-gradient(135deg, rgba(0, 0, 0, 0.9), rgba(30, 30, 30, 0.9))';
        });
    } else {
        document.body.style.background = 'linear-gradient(135deg, #667eea 0%, #764ba2 25%, #f093fb 50%, #f5576c 75%, #4facfe 100%)';
        document.body.style.color = '#ffffff';

        // Reset all premium sections
        document.querySelectorAll('.premium-settings-section').forEach(section => {
            section.style.background = 'rgba(255, 255, 255, 0.1)';
            section.style.borderColor = 'rgba(102, 126, 234, 0.2)';
        });
        ...
        // Reset modals
        document.querySelectorAll('.premium-modal-content').forEach(modal => {
            modal.style.background = 'linear-gradient(135deg, rgba(255, 255, 255, 0.2), rgba(255, 255, 255, 0.1))';
        });
    }
}
...
document.getElementById('profilePicInput').addEventListener('change', async (e) => {
    ...
    if (file) {
        try {
            ...
            if (error) {
                console.error('Error updating profile picture in Supabase:', error);
            }
...
async function saveProfile() {
    ...
    if (!username) {
        ...
        document.getElementById('profileUsername').value = username.replace('ZAO-', '');
    }
    ...
    // Validate username pattern (4-12 characters after ZAO-)
    const usernameWithoutPrefix = username.replace('ZAO-', '');
    const usernamePattern = /^[a-zA-Z0-9_]{4,12}$/;
    if (!usernamePattern.test(usernameWithoutPrefix)) {
        showNotification('Username must be 4-12 characters (letters, numbers, underscore only)', 'error');
        return;
    }
...
function shareInviteLink() {
    ...
    const deploymentUrl = 'https://mceeomoba112.github.io/ZAO-APP-';
    const inviteLink = `${deploymentUrl}/index.html?invite=${inviteCode}&ref=${username}`;
    const inviteText = `🎉 Join me on ZAO APP! 
...
function showQRCode() {
    ...
    qrContainer.innerHTML = `
        <div class="qr-display" style="text-align: center; padding: 2rem;">
            ...
            <h4 style="color: white; margin: 1rem 0 0.5rem; font-size: 1.1rem;">Scan to Connect</h4>
            <p style="color: rgba(255,255,255,0.8); font-size: 0.9rem; margin: 0;">Invite Code: <strong>${inviteCode}</strong></p>
            <p style="color: rgba(255,255,255,0.7); font-size: 0.8rem; margin: 0.5rem 0 0; word-break: break-all;">${inviteLink}</p>
        </div>
    `;
